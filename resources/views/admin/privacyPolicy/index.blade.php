        ...
        <div class="container-xxl">
            ...
            <div class="py-3 d-flex align-items-sm-center flex-sm-row flex-column">
                <div class="flex-grow-1">
                    <h4 class="fs-18 fw-semibold m-0">Privacy Policy List</h4>
                </div>

                <div class="text-end">
                    <ol class="breadcrumb m-0 py-0">
                        <a href="{{ route('admin.privacy-policy.create') }}" class="btn btn-success">New Privacy Policy
                            Create</a>
                    </ol>
                </div>
            </div>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <table id="responsive-datatable"
                                ...
                                <tbody>
                                    @foreach ($privacys as $key => $item)
                                        <tr>
                                            <td>{{ $key + 1 }}</td>
                                            <td>{{ $item->heading }}</td>
                                            <td>
                                                {{ Str::words($item->content, 8, '......') }}
                                            </td>
                                            <td>
                                                <a href="javascript:void(0);"
                                                    class="toggle-status  {{ $item->status ? 'badge text-bg-primary' : 'badge text-bg-secondary' }}"
                                                    data-url="{{ route('admin.privacy-policy.active.deactive', $item->id) }}"
                                            ...
                                            <td>
                                                <a href="{{ route('admin.privacy-policy.details', $item->id) }}"
                                                    class="btn btn-primary">View</a>
                                                <a href="javascript:void(0);" class="btn btn-danger delete-btn"
                                                    data-url="{{ route('admin.privacy-policy.destroy', $item->id) }}">Delete</a>
                                        ...
                                        <div class="modal fade" id="editBannerModal{{ $item->id }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-xl">
                                                <div class="modal-content">
                                                    <form action="{{ route('admin.privacy-policy.update', $item->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="mb-3">
                                                                ...
                                                                <input type="text" name="heading" id="example-email"
                                                            ...
                                                            <div class="mb-3">
                                                                <label for="example-textarea" class="form-label">Content
                                                                    <small class="text-danger">

                                                                    </small></label>
                                                                <textarea name="content" class="form-control @error('description') is-invalid @enderror" id="example-textarea"
                                                                    rows="17" spellcheck="false" placeholder="Content">{{ old('content', $item->content) }}</textarea>
                                                                @error('content')
                                                                    <span class="text-danger"
                                                                        style="font-size: 14px;">{{ $message }}</span>
                                                                @enderror
